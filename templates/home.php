<?php /* Template Name: Home Page */ ?>

<?php get_header(); ?>

<main id="template_body" class="home_template">
    

    <section class="hero hero-s6">
        <figure class="bg">
            <img src="<?php echo MBN_ASSETS_URI ?>/img/banner_hero.jpg" alt="<?php echo get_bloginfo('name'); ?>" />
        </figure>
        <div class="hero-caption middle">
            <div class="grid-container"> 
                <div class="grid-x grid-margin-x">
                    <div class="cell large-9 align-self-middle">
                        <h1>Specializing in saving the lives of </h1>
                        <h2>large breed, pregnant, and nursing dogs.</h2>
                        
                        <a href="<?php the_permalink(222); ?>" class="button with_icon">
                            <img src="<?php echo MBN_ASSETS_URI ?>/img/icons/icon_donate.svg" alt="Donate" /> 
                            <span>Donate</span>
                        </a>
                        <a href="<?php the_permalink(100); ?>" class="button with_icon">
                            <img src="<?php echo MBN_ASSETS_URI ?>/img/icons/icon_volunteer.svg" alt="Volunteer" /> 
                            <span>Volunteer</span>
                        </a>
                        <a href="<?php the_permalink(141); ?>" class="button with_icon">
                            <img src="<?php echo MBN_ASSETS_URI ?>/img/icons/icon_foster.svg" alt="Foster" /> 
                            <span>Foster</span>
                        </a>
                        <a href="<?php the_permalink(220); ?>" class="button with_icon">
                            <img src="<?php echo MBN_ASSETS_URI ?>/img/icons/icon_adopt.svg" alt="Adopt" /> 
                            <span>Adopt</span>
                        </a>
                    </div>
                </div>
            </div>
        </div>
    </section>


     <section class="cta cta-s2 page_section section_relative">
        <figure class="cta-bg"><!-- <img src="assets/img/cta-2.jpg" alt=""> --></figure>
        <img class="bg_left bg_dogs" src="<?php echo MBN_ASSETS_URI ?>/img/dog1.jpg" alt="Almost There Rescue" />
        <img class="bg_right bg_dogs" src="<?php echo MBN_ASSETS_URI ?>/img/dog2.jpg" alt="Almost There Rescue" />
        <div class="grid-container">
            <div class="grid-x grid-margin-x">
                <div class="cell large-8 large-offset-2">
                    <div class="cta-body">
                        <h3>Meet Your <span>New Best Friend</span></h3>

                        <p><strong>Ready to take your heart for a spin?</strong> Just head over to our <img src="<?php echo MBN_ASSETS_URI ?>/img/petfinder_text.png" alt="Petfinder" />  page to see all of our adorable pups looking for furever homes. Almost There Rescue has a pretty constant flow of puppies and adult dogs of all kinds who are guaranteed to take your breath away.</p>

                        <p>
                            <a href="http://awos.petfinder.com/shelters/almostthere.html" target="_blank" class="button purple with_icon">
                                <img src="<?php echo MBN_ASSETS_URI ?>/img/icons/petfinder.png" alt="Petfinder" /> 
                                <span>View Adoptable Dogs on Petfinder</span>
                            </a>
                        </p>

                        <div class="keep_in_touch">
                            <p>
                                <strong>We’d love to keep in touch!</strong> Follow us on:                                 
                                <a href="https://www.facebook.com/almostthererescue" target="_blank">
                                    <img src="<?php echo MBN_ASSETS_URI ?>/img/logo-text-facebook.png" alt="Facebook" /></a> 
                                <a href="https://www.instagram.com/almostthererescue/" target="_blank">
                                    <img src="<?php echo MBN_ASSETS_URI ?>/img/logo-text-instagram.png" alt="Instagram" /></a>
                                
                            </p>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </section>


    <section class="page_section service_options">
        <div class="grid-container">
            <div class="grid-x align-center">
                <div class="cell service_cell small-6 medium-2 large-2">
                    <div class="service_box_wrapper">
                        <a href="<?php the_permalink(222); ?>">
                            <svg xmlns="http://www.w3.org/2000/svg" width="62.531" height="62.544"><defs></defs><g id="donate_icon" transform="translate(-.054)"><g id="Group_834" transform="translate(.054 36.891)"><g id="Group_833"><path id="Path_1364" d="M5.8 302H1.886a1.831 1.831 0 00-1.832 1.832v21.988a1.831 1.831 0 001.832 1.832H5.8a5.5 5.5 0 005.5-5.5V307.5a5.5 5.5 0 00-5.5-5.5z" class="cls-1" transform="translate(-.054 -302)"/></g></g><g id="Group_836" transform="translate(14.957 36.189)"><g id="Group_835"><path id="Path_1365" d="M169.147 300.5a3.355 3.355 0 00-.843-.916 3.631 3.631 0 00-4.947.367l-8.319 9.6-.257.293a5.407 5.407 0 01-4.031 1.759h-14.218a1.832 1.832 0 110-3.665h11.177a3.675 3.675 0 003.665-3.665v-.037a3.668 3.668 0 00-3.665-3.628h-6.633a5.044 5.044 0 01-3.225-1.209 12.843 12.843 0 00-15.795-.843V321.1a42.3 42.3 0 0011.141 1.5h16.345a12.844 12.844 0 0010.261-5.131l8.8-11.727a4.853 4.853 0 00.544-5.242z" class="cls-1" transform="translate(-122.056 -296.246)"/></g></g><g id="Group_838" transform="translate(12.148)"><g id="Group_837"><path id="Path_1366" d="M136.093 3.542a10.438 10.438 0 00-16.56 1.164c-.165.231-.32.462-.464.69-.145-.229-.3-.459-.464-.69a10.438 10.438 0 00-16.56-1.164 12.216 12.216 0 00-2.991 8.158c0 3.362 1.3 6.465 4.106 9.765a88.673 88.673 0 0010.009 9.264c1.5 1.266 3.051 2.575 4.7 4a1.832 1.832 0 002.4 0c1.648-1.428 3.2-2.738 4.7-4 2.607-2.2 4.859-4.1 6.812-5.944 3.793-3.579 7.3-7.641 7.3-13.085a12.216 12.216 0 00-2.988-8.158z" class="cls-1" transform="translate(-99.054)"/></g></g></g></svg>
                             <span>Donate</span></a>
                    </div>
                </div>
                <div class="cell service_cell small-6 medium-2 large-2">
                    <div class="service_box_wrapper">
                        <a href="<?php the_permalink(100); ?>">
                            <svg xmlns="http://www.w3.org/2000/svg" width="74.023" height="69.427"><defs></defs><g id="volunteer_icn" transform="translate(0 -15.895)"><path id="Path_1367" d="M131.833 79.864c2.228-2.05 2.459-5.722.54-8.541a6.216 6.216 0 10-9.1 8.3c2.578 2.092 6.26 2.31 8.56.241z" class="cls-1" transform="translate(-103.511 -45.088)"/><path id="Path_1368" d="M221.976 16.1a6.147 6.147 0 00-4.246 8.357c1.286 3.077 4.429 5.009 7.463 4.25a6.6 6.6 0 004.479-7.237 6.16 6.16 0 00-7.696-5.37z" class="cls-1" transform="translate(-185.838)"/><path id="Path_1369" d="M340.525 24.5a6.215 6.215 0 10-11.941-2.995 6.612 6.612 0 004.53 7.256c2.764.699 6.031-.956 7.411-4.261z" class="cls-1" transform="translate(-281.045 -.038)"/><path id="Path_1370" d="M435 70.273a6.176 6.176 0 00-9.306 1.072c-1.923 2.819-1.69 6.491.557 8.56 2.3 2.069 5.988 1.812 8.539-.258a6.16 6.16 0 00.21-9.374z" class="cls-1" transform="translate(-363.037 -45.113)"/><path id="Path_1371" d="M200.5 135.923c-10.315 0-20.482 6.682-19.448 16.471a10.818 10.818 0 0016.411 7.963 5.88 5.88 0 016.074 0 10.964 10.964 0 0016.556-7.961c1.031-9.758-9.276-16.473-19.593-16.473z" class="cls-1" transform="translate(-154.815 -102.674)"/><path id="Path_1372" d="M7.76 342.524l1.94-.97c2.027-1.01 4.473.034 8.326 1.364 4.346 1.5 9.579 3.3 16.021 3.3a40.182 40.182 0 0018.188-4.666l9.7-4.85-1-2c-1.619-3.181-5.676-4.281-8.971-2.908a47.574 47.574 0 01-18.26 3.988c-6.733 0-10.651-1.807-10.99-1.97l1.87-3.914c.092.04 5.733 2.512 15.311.907a25.945 25.945 0 00-26.32-9.211A51.235 51.235 0 000 327.006z" class="cls-1" transform="translate(0 -260.897)"/></g></svg> <span>Volunteer</span></a>
                    </div>
                </div>
                <div class="cell service_cell small-6 medium-2 large-2">
                    <div class="service_box_wrapper">
                        <a href="<?php the_permalink(141); ?>">
                            <svg xmlns="http://www.w3.org/2000/svg" width="75.377" height="75.377"><defs></defs><g id="foster_icn"><g id="Group_839" transform="translate(58.85 65.072)"><path id="Path_1373" d="M416.027 448.2a10.232 10.232 0 00-9.4-6.2h-3.9v2.208a12.455 12.455 0 01-2.984 8.1h13.577a2.944 2.944 0 002.707-4.108z" class="cls-1" transform="translate(-399.741 -442)"/></g><path id="Path_1374" d="M53.3 43.494A24.093 24.093 0 0042 34.45a19.683 19.683 0 01-11.96-13.416 7.911 7.911 0 01-4.047 1.1c-.256 0-.518-.012-.776-.037a7.86 7.86 0 01-7.122-6.7l-.523-3.349a2.208 2.208 0 114.364-.682l.523 3.349a3.458 3.458 0 003.167 2.985c.121.011.243.017.363.017a3.446 3.446 0 003.31-2.355l1.621-4.538.006-.015A8.1 8.1 0 0023.3 0h-6.709A8.113 8.113 0 0010.6 2.65L5.648 8.1h-3.4a2.27 2.27 0 00-.967.2A2.212 2.212 0 000 10.306v6.624a5.172 5.172 0 005.162 5.153 3.688 3.688 0 013.655 3.33l-.531 1.436a23.188 23.188 0 002.02 20.17v19.9a4.417 4.417 0 00-4.417 4.417v1.83A2.208 2.208 0 008.1 75.377h11.04V42.694a2.208 2.208 0 114.417 0v32.683H26.5v-4.416a6.632 6.632 0 016.625-6.625h2.43a11.061 11.061 0 0110.82-8.836 2.208 2.208 0 010 4.417 6.632 6.632 0 00-6.625 6.625 2.208 2.208 0 01-2.208 2.208h-4.417a2.211 2.211 0 00-2.208 2.208v4.417h18.4a8.1 8.1 0 008.1-8.1V57.212A24.557 24.557 0 0053.3 43.494z" class="cls-1"/></g></svg>
                         <span>Foster</span></a>
                    </div>
                </div>
                <div class="cell service_cell small-6 medium-2 large-2">
                    <div class="service_box_wrapper">
                        <a href="<?php the_permalink(86); ?>">
                            <svg xmlns="http://www.w3.org/2000/svg" id="employment" width="68.896" height="75.377"><defs></defs><path id="Path_1390" d="M357.482 178.775s-6.5.78-5.316 14.863c1.263 15.066 6.859 14.526 7.615 14.549.874.026 5.795-.268 7.544-3.215 1.231-2.074 2.7-14.078-1.757-20.841-3.546-5.381-6.875-5.831-8.086-5.356z" class="cls-1" transform="translate(-352.022 -160.319)"/><path id="Path_1391" d="M402.277 138.935a6.427 6.427 0 00-9.857 1.987 11.341 11.341 0 00-.916 2.7c-1.264 5.933-1.372 19.392 2.279 23.8a7.332 7.332 0 009.068 1.91c.732-.311 6.134-2.313 5.682-12.944-.348-8.178-2.633-14.463-6.256-17.453z" class="cls-1" transform="translate(-373.512 -137.434)"/><path id="Path_1392" d="M446.69 144.494a4.032 4.032 0 00-3.89.225c-2.666 1.666-7.432 6.223-8.277 17.762-.844 11.5 6.094 11.18 10.178 10.032a6.214 6.214 0 003.976-3.459c1.234-2.761 3.036-7.266 2.972-9.917-.086-3.609.432-12.037-4.959-14.643z" class="cls-1" transform="translate(-397.811 -141.136)"/><path id="Path_1393" d="M479.879 185.392s-7.283 1.5-7.887 11.269.082 13.54 2.724 15.233c1.654 1.059 7.483 1.486 9.489-2.841 1.908-4.116 3.706-9.808 3.133-14.377-.701-5.597-4.208-10.032-7.459-9.284z" class="cls-1" transform="translate(-418.551 -164.028)"/><g id="Group_842" transform="translate(4.317 33.065)"><path id="Path_1398" d="M413.657 230.075c-6.876-4.194-5.244-7.569-9.957-13.789-4.14-5.463-7.8-4.139-13.677-4.456s-10.828 6.768-11.274 7.522c-2.07 3.5-1.232 5.247-7.963 8.352a17.156 17.156 0 00-8.848 9.363c-1.047 6.653 2.134 10.884 4.9 12.906 2.332 1.707 9.363 1.513 15.247 1.053a8.584 8.584 0 01-1.437-1.441q-3.458-4.4-3.215-11.8a26.772 26.772 0 013.761-13.467q4.216-6.854 11.4-6.855 7.492 0 10.828 3.791 2.7 3.094 4.277 11.161a31.29 31.29 0 01.606 5.945 16.651 16.651 0 01-2.911 10.009 12.93 12.93 0 01-3.938 3.631 93.682 93.682 0 0011.388 2.106c5.505.461 9.313-4.405 9.839-10.54.451-5.266-1.935-9.166-9.026-13.491z" class="cls-1" transform="translate(-361.734 -211.82)"/><path id="Path_1399" d="M423.136 246.628q-2.547 0-4.7 3.4a11.155 11.155 0 00-1.942 5.972 10.645 10.645 0 001.759 5.611q1.971 3.215 4.55 3.215 2.7 0 5.247-3.276a9.8 9.8 0 002.4-5.915 10.74 10.74 0 00-2.851-6.764q-2.37-2.243-4.463-2.243z" class="cls-1" transform="translate(-392.153 -231.156)"/></g></svg> <span>Employment</span></a>
                    </div>
                </div>
                <div class="cell service_cell small-6 medium-2 large-2">
                    <div class="service_box_wrapper">
                        <a href="<?php the_permalink(139); ?>">
                            <svg xmlns="http://www.w3.org/2000/svg" width="80" height="73.226"><defs><style>.cls-2{fill:#348f41}</style></defs><g id="events" transform="translate(14634.323 7395.021)"><g id="appointment_1_" transform="translate(-14634.323 -7388.069)"><g id="Group_854" transform="translate(0 15.21)"><path id="Path_1431" d="M118.667 199.087H58.011a9.673 9.673 0 01-9.672-9.672v-41.392h80v41.392a9.672 9.672 0 01-9.672 9.672z" class="cls-1" transform="translate(-48.339 -148.023)"/></g><g id="Group_855" transform="translate(31.222 37.097)"><path id="Path_1433" d="M136.921 207.523a3.99 3.99 0 00-3.637-4.056 3.844 3.844 0 00-4.05 3.838v.217a4.19 4.19 0 01-1.838 3.4 3.987 3.987 0 00-1.64 3.881 4.031 4.031 0 003.132 3.309 4.078 4.078 0 002.054-.1 6.978 6.978 0 014.271 0 4.073 4.073 0 002.054.1 4.032 4.032 0 003.133-3.312 3.988 3.988 0 00-1.64-3.881 4.189 4.189 0 01-1.839-3.396z" class="cls-2" transform="translate(-124.007 -201.758)"/><path id="Path_1434" d="M127.426 217.6a5.985 5.985 0 01-1.174-.117 5.771 5.771 0 01-4.478-4.725 5.7 5.7 0 012.34-5.523 2.484 2.484 0 001.119-2.012V205a5.566 5.566 0 011.729-4.022 5.469 5.469 0 014.112-1.516 5.659 5.659 0 015.251 5.755 2.482 2.482 0 001.119 2.011 5.7 5.7 0 012.34 5.523 5.77 5.77 0 01-4.478 4.724 5.837 5.837 0 01-2.907-.148 5.247 5.247 0 00-3.24 0 5.728 5.728 0 01-1.733.273zm3.354-14.735a2.142 2.142 0 00-2.142 2.135v.218a5.923 5.923 0 01-2.555 4.788 2.276 2.276 0 00-.941 2.239 2.34 2.34 0 001.785 1.895 2.374 2.374 0 001.2-.057 8.718 8.718 0 015.3 0 2.311 2.311 0 002.987-1.838 2.278 2.278 0 00-.941-2.24 5.92 5.92 0 01-2.555-4.787 2.29 2.29 0 00-2.025-2.356h-.116z" class="cls-2" transform="translate(-121.71 -199.456)"/></g><g id="Group_856" transform="translate(48.295 35.936)"><circle id="Ellipse_14" cx="3.558" cy="3.558" r="3.558" class="cls-2" transform="translate(1.702 1.702)"/><path id="Path_1435" d="M167.09 207.249a5.26 5.26 0 115.261-5.26 5.265 5.265 0 01-5.261 5.26zm0-7.116a1.856 1.856 0 101.857 1.856 1.858 1.858 0 00-1.857-1.856z" class="cls-2" transform="translate(-161.831 -196.729)"/></g><g id="Group_857" transform="translate(41.662 25.989)"><circle id="Ellipse_15" cx="3.558" cy="3.558" r="3.558" class="cls-2" transform="translate(1.703 1.702)"/><path id="Path_1436" d="M151.506 183.873a5.26 5.26 0 115.261-5.26 5.267 5.267 0 01-5.261 5.26zm0-7.117a1.856 1.856 0 101.857 1.857 1.859 1.859 0 00-1.857-1.857z" class="cls-2" transform="translate(-146.245 -173.352)"/></g><g id="Group_858" transform="translate(30.058 25.989)"><circle id="Ellipse_16" cx="3.558" cy="3.558" r="3.558" class="cls-2" transform="translate(1.702 1.702)"/><path id="Path_1437" d="M124.235 183.873a5.26 5.26 0 115.261-5.26 5.265 5.265 0 01-5.261 5.26zm0-7.117a1.856 1.856 0 101.857 1.857 1.858 1.858 0 00-1.857-1.857z" class="cls-2" transform="translate(-118.976 -173.352)"/></g><g id="Group_859" transform="translate(21.769 35.936)"><circle id="Ellipse_17" cx="3.558" cy="3.558" r="3.558" class="cls-2" transform="translate(1.702 1.702)"/><path id="Path_1438" d="M104.756 207.249a5.26 5.26 0 115.259-5.26 5.267 5.267 0 01-5.259 5.26zm0-7.116a1.856 1.856 0 101.855 1.856 1.858 1.858 0 00-1.855-1.856z" class="cls-2" transform="translate(-99.495 -196.729)"/></g><g id="Group_860" transform="translate(11.915)"><path id="Path_1439" d="M83.743 94.3a3.4 3.4 0 01-3.4-3.4V80.683a3.4 3.4 0 016.809 0V90.9a3.4 3.4 0 01-3.409 3.4z" class="cls-1" transform="translate(-78.637 -75.577)"/><path id="Path_1440" d="M81.445 93.7a5.112 5.112 0 01-5.106-5.1V78.385a5.106 5.106 0 0110.213 0V88.6a5.112 5.112 0 01-5.107 5.1zm0-17.021a1.7 1.7 0 00-1.7 1.7V88.6a1.7 1.7 0 103.4 0V78.385a1.7 1.7 0 00-1.7-1.702z" class="cls-1" transform="translate(-76.339 -73.279)"/></g><g id="Group_861" transform="translate(57.873)"><path id="Path_1441" d="M191.743 94.3a3.4 3.4 0 01-3.4-3.4V80.683a3.4 3.4 0 116.809 0V90.9a3.4 3.4 0 01-3.409 3.4z" class="cls-1" transform="translate(-186.637 -75.577)"/><path id="Path_1442" d="M189.445 93.7a5.112 5.112 0 01-5.106-5.106V78.385a5.106 5.106 0 0110.213 0V88.6a5.112 5.112 0 01-5.107 5.1zm0-17.021a1.7 1.7 0 00-1.7 1.7V88.6a1.7 1.7 0 003.4 0V78.385a1.7 1.7 0 00-1.7-1.702z" class="cls-1" transform="translate(-184.339 -73.279)"/></g></g><path id="Rectangle_7708" d="M0 0h79.589v9.949H0z" class="cls-1" transform="translate(-14634.094 -7395.021)"/></g></svg>
                         <span>Events</span></a>
                    </div>
                </div>
            </div>
        </div>
    </section>

    <section class="page_section image_carousel">
        <div class="grid-container full">
            <div class="grid-x dogs_slider">
                <div class=""><img src="<?php echo MBN_ASSETS_URI ?>/img/sample1.jpg" alt="Almost There Rescue" /></div>
                <div class=""><img src="<?php echo MBN_ASSETS_URI ?>/img/sample2.jpg" alt="Almost There Rescue" /></div>
                <div class=""><img src="<?php echo MBN_ASSETS_URI ?>/img/sample3.jpg" alt="Almost There Rescue" /></div>
                <div class=""><img src="<?php echo MBN_ASSETS_URI ?>/img/sample4.jpg" alt="Almost There Rescue" /></div>
                <div class=""><img src="<?php echo MBN_ASSETS_URI ?>/img/sample2.jpg" alt="Almost There Rescue" /></div>
                <div class=""><img src="<?php echo MBN_ASSETS_URI ?>/img/sample3.jpg" alt="Almost There Rescue" /></div>
            </div>
        </div>
    </section>

    <div class="page_section map_section">
        <div class="grid-container full">
            <div class="grid-x">
                <div class="cell large-6 text_content">
                    <h2>Visiting & Adopting from Almost There: <strong>A Mom + Pups Rescue</strong></h2>

                    <p><strong>Interested in turning one of our pups into a member of your family? Come and visit us at our facility!</strong></p>

                    <p>Appointments slots are one hour long and are on a first come, first served basis. Upon arrival, we will ask that you fill out our Adoption Questionnaire and wait to be seen by the next available Matchmaker. We look forward to your visit, and can’t wait for you to fall in love!</p>

                    <ul>
                        <li><img src="<?php echo MBN_ASSETS_URI ?>/img/icons/pin.svg" alt="Location" /> <strong>2601 E Indian School Road</strong></li>
                        <li><img src="<?php echo MBN_ASSETS_URI ?>/img/icons/clock.svg" alt="Office Hours" /> <strong>Weekdays from 11am-8pm and weekends from 8:00am-8:00pm</strong></li>
                    </ul>


                </div>
                <div class="cell large-6 map_content">
                    <!-- <iframe src="https://snazzymaps.com/embed/265816" width="100%" height="750px" style="border:none;"></iframe> -->
                        <iframe src="https://snazzymaps.com/embed/271250" width="100%" height="750px" style="border:none;"></iframe>    
                </div>
            </div>
        </div>
    </div>

    <div class="page_section sign_up_section">
        <div class="grid-container full">
            <div class="grid-x align-right">
                <div class="cell sign_up_image">
                    <img src="<?php echo MBN_ASSETS_URI ?>/img/signup_banner.jpg" alt="Signup" />
                </div>
                <div class="cell medium-12 large-7 text_content">
                    <h2>Sign Up Fur Updates!</h2>
                    <p>We’d love to keep in touch! Follow us on Facebook, Instagram, or sign up for our mailing list to receive updates, view upcoming adoptable pups, and get a glimpse of behind the scenes action! <br> <small>(We promise we won’t hound you)</small></p>
                   
                    <p>
                        <a href="#https://visitor.r20.constantcontact.com/d.jsp?llr=s4zzvz8ab&p=oi&m=1129364383730&sit=nfskxt5lb&f=712a7eff-7e76-437d-a073-393c8084151b" target="_blank" class="button with_icon green">
                            <img src="<?php echo MBN_ASSETS_URI ?>/img/icons/icon_email.svg" alt="Join Our Mailing List" /> 
                            <span>Join Our Mailing List</span>
                        </a>
                    </p>
                </div>
            </div>
        </div>
    </div>

    <div class="page_section amazon_section">
        <div class="grid-container">
            <div class="grid-x">
                <div class="cell large-12 amazon_wishlist">
                    <div class="grid-x align-center">
                        <div class="cell medium-8 amazon_wishlist_content">

                            <p><img src="<?php echo MBN_ASSETS_URI ?>/img/wishlist-amazon.png" alt="Wishlist" /></p>
                    
                            <p>Click here to see our Amazon Wishlist where you can purchase items we desperately need from Amazon.com and have it shipped directly to Almost There Rescue. Thank you! We appreciate your support!</p>

                            <p>
                                <a href="https://www.amazon.com/gp/registry/wishlist/1OARMMRA4QUBB" targe="_blank" class="button with_icon green">
                                    <img src="<?php echo MBN_ASSETS_URI ?>/img/icons/icon_shop.svg" alt="Shop Our Wishlist" /> 
                                    <span>Shop Our Wishlist</span>
                                </a>
                            </p>

                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>


    <div class="page_section ig_feed">
        <div class="grid-container">
            <div class="grid-x ig_title">
                <div class="cell large-12">
                    <h2><a href="https://www.instagram.com/almostthererescue/" target="_blank">@almostthererescue</a></h2>
                </div>
            </div>
            <div class="grid-x">
                <div class="cell large-12">
                    <?php echo do_shortcode("[instagram-feed]"); ?>
                </div>
            </div>
        </div>
    </div>




</main>

<?php get_footer(); ?>